...
class GLRModule:
	def __init__(self, selection_module, num_arm, delta, globalRestart, lazyness, dimensions=1):
	...
	def thisHappened(self, arm, reward, t):
		reset_pending = False
		# For each dimension of the pulled arm:
		for dimension in range(self.dimensions):
			index = dimension + arm*self.dimensions
			...
			if self.dimensions > 1:
				...
				rwd = reward
			self.past_rewards[index].append(rwd)
			self.past_rewards_sums[index] += rwd
			
			n=len(self.past_rewards[index])
			if n > 2 and n%self.lazyness == 0:
				...
				mean = self.past_rewards_sums[index] / len(self.past_rewards[index])
				left_mean = 0
				right_mean = mean
				supreme = -1
				for s in range(1, n-1):
					z = self.past_rewards[index][s-1]
				...
				if supreme >= self.betaFunction(len(self.past_rewards[index]), self.delta):
					print("Detected change in dimension", dimension, "of arm", arm, "at timestep", t, "after", len(self.past_rewards[index]), "pulls")
					print("supreme was", supreme, "and result of beta was", self.betaFunction(len(self.past_rewards[index]), self.delta))
					reset_pending = True
		if reset_pending:
			self.restart(arm)
	...
	def fullReset(self):
		...
		self.past_rewards_sums = [0]*self.num_arm*self.dimensions
	...
	def restart(self, arm):
		if self.globalRestart:
			...
			for index in range(arm*self.dimensions, (arm+1)*self.dimensions):
				self.past_rewards[index] = list()
				self.past_rewards_sums[index] = 0
