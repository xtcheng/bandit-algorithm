        ...
        for i in range(self.T):
            for agent in range(self.num_agents):
                ...
                for neighbor in range(self.num_agents):
                    if self.observation_matrix[agent][neighbor] and round(self.sociabilities[agent] == 1):
                        self.update(agent, arm, rwd, br, i)
    # Q! As stated from the paper that agent observes neighbours based on the sociability value, however the value is assessed randomly, but can also be deterministic by adding a rule, rounding up to one, this could imply anything above 0.5 would be highly socialable and below they dont interact with neighbours, which could induce ambiguity...np.random.rand() < self.sociabilities[agent] 
